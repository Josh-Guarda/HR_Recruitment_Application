...
#         [(brgy['brgyCode'], brgy['brgyDesc']) for brgy in BARANGAY_DATA],
#         key=lambda x: x[0].lower()  # Sort by barangay name (case-insensitive)
#     )

#     form.munci_id.choices = [('', '-- Select Municipality --')] + sorted(
#         [(muni['citymunCode'], muni['citymunDesc']) for muni in MUNICIPALITY_DATA],
#         key=lambda x: x[0].lower()  # Sort by municipality name
#     )

#     form.prov_id.choices = [('', '-- Select Province --')] + sorted(
#         [(prov['provCode'], prov['provDesc']) for prov in PROVINCE_DATA],
#         key=lambda x: x[0].lower()  # Sort by province name
#     )
...
#         # current_user.brgy_name = form.brgy_name.data
...
# @app.route('/search/barangay')
# def search_barangay():
#     term = request.args.get('term', '').lower()
#     with open(Barangay_JSON, encoding='utf-8') as f:
#         records = json.load(f)['RECORDS']

#     # Filter barangays by keyword
#     matches = [
#         {'id': brgy['brgyCode'], 'text': brgy['brgyDesc']}
#         for brgy in records
#         if term in brgy['brgyDesc'].lower()
#     ]
